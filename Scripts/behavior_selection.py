"""
Filename: behavior_selection.py
...
def apply_clustering_and_plot(predictors_func: pd.DataFrame, labels_func: pd.DataFrame, cluster_algorithms_func: dict, name_func: str):
    """
    ...
    for algo_name, algo in cluster_algorithms_func.items():
        ...
        if 'n_clusters' in algo.get_params():
            for i_fu in num_clusters:
                ...
                plt_func.plot_clustering_results(clusters, labels_func, scaled_predictors, pca_components, i_fu + 2, algo_name, pdf, name_func=name_func)
        else: 
            print('no n-clusters')
            # clustering and plotting if no number of clusters input needed 
            clusters = algo.fit_predict(scaled_predictors)
            plt_func.plot_clustering_results(clusters, labels_func, scaled_predictors, pca_components, len(clusters), algo_name, pdf, name_func=name_func)

def apply_weighted_clustering_and_plot(predictors_func: pd.DataFrame, labels_func: pd.DataFrame, weights_func: pd.DataFrame, name_func: str, option_gen: str):
    """
    Function to apply different clustering algorithms on the data. 
    ...
    for w in range(weights_func.shape[0]):
        ...
        if option_gen == 'optimization':
            ...
            plt_func.plot_clustering_results(clusters, labels_func, scaled_predictors, pca_components, num_clusters, 'KMeans', pdf, add=title_addition, name_func=name_func)

            clusters = SpectralClustering(n_clusters=num_clusters, random_state=42, affinity='nearest_neighbors').fit_predict(scaled_predictors)
            plt_func.plot_clustering_results(clusters, labels_func, scaled_predictors, pca_components, num_clusters, 'Spectral Clustering', pdf, add=title_addition, name_func=name_func)

            clusters = GaussianMixture(n_components=5, random_state=42).fit_predict(scaled_predictors)
            plt_func.plot_clustering_results(clusters, labels_func, scaled_predictors, pca_components, num_clusters, 'Gaussian Mixture', pdf, add=title_addition, name_func=name_func)

        else:
            clusters = GaussianMixture(n_components=5, random_state=42).fit_predict(scaled_predictors)
            plt_func.plot_clustering_results_only_table(clusters, labels_func, scaled_predictors, pca_components, num_clusters,
    ...
    with PdfPages(output_pdf_path) as pdf:
        for filepaths_temp in filepaths:
            ...
            if option == 'different k - not weighted':
                ...
                apply_weighted_clustering_and_plot(predictors, labels, weights, name_func=name, option_gen = option_gen)
        ...
        if option == 'different k - not weighted':
            ...
            weights = weights.drop([1,3,5], axis=0)
            apply_weighted_clustering_and_plot(predictors, labels1, weights, name_func=name, option_gen = option_gen)
            apply_weighted_clustering_and_plot(predictors, labels, weights, name_func=name, option_gen = option_gen)
